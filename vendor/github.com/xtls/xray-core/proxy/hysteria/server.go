...
func (s *Server) Process(ctx context.Context, network net.Network, conn stat.Connection, dispatcher routing.Dispatcher) error {
	...
	// TryUnwrapStatsConn is needed because tcpWorker may wrap conn in a
	// CounterConnection (for inbound-level stats) before calling Process.
	// CounterConnection does not forward User(), so we must unwrap first.
	connToCheck := stat.TryUnwrapStatsConn(conn)
	if v, ok := connToCheck.(User); ok {
		inbound.User = v.User()
		if inbound.User != nil {
